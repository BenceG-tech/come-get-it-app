...
- 18 venue drinks, 19 free-drink windows, 1 active reward row, and 25 redemption records. The reward's venue is paused, so there are currently zero customer-visible rewards.
- `create-redemption-window` version 6, `confirm-redemption` version 6, `delete-account` version 2, and `redeem-reward` version 37 are deployed with JWT verification enabled.
...
5. **CSR and live reward content:** all five active venues currently have CSR disabled and no default charity. Configure real charity records and venue donation settings before promoting impact claims. Also create at least one genuine reward for an active venue before producing reward-focused screenshots; the only existing reward belongs to a paused venue and is intentionally hidden.
