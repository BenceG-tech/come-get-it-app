-- Consumers may read only rewards that are currently redeemable. Venue staff
-- and administrators retain their broader management policies.
drop policy if exists "Authenticated users can view available rewards" on public.rewards;
...
using (
  active = true
  and valid_until >= (now() at time zone 'Europe/Budapest')::date
  and (
    max_redemptions is null
    or coalesce(current_redemptions, 0) < max_redemptions
  )
  and (
    ...
    or exists (
      select 1
       ...
       where venue.id in (rewards.venue_id, rewards.partner_id)
         and venue.is_paused = false
    )
  )
);

-- Keep the write path aligned with the read policy. A caller cannot redeem a
-- hidden reward by guessing its UUID after the venue has been paused.
